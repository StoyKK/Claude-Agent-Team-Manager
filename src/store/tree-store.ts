...
export const useTreeStore = create<TreeStore>()((set, get) => ({
  ...
  async loadProject(path: string) {
    ...
    try {
      ...
      try {
        ...
        logger.warn("TreeStore", "Could not create .aui directory: " + String(dirErr));
      }
      ...
      for (const filePath of filePaths) {
        ...
        try {
          ...
          logger.warn("TreeStore", "Failed to parse file: " + filePath + " " + String(e));
        }
      }
      ...
      logger.info("TreeStore", `Loaded ${nodes.size} nodes (${skillCount} skills, ${skillNameCache.size} cached names)`);
      ...
      try {
        ...
        logger.warn("TreeStore", "Failed to save metadata during load: " + String(saveErr));
      }
      ...
      try {
        ...
        logger.warn("TreeStore", "Failed to load layouts: " + String(layoutErr));
      }
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      logger.error("TreeStore", "loadProject failed: " + String(err));
      set({
  ...
  async saveTreeMetadata() {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      logger.error("TreeStore", "saveTreeMetadata failed: " + String(err));
...
`;
      ...
      for (const [, node] of nodes) {
        if (node.kind === "skill" && node.sourcePath) {
          try {
            ...
            logger.warn("TreeStore", "Could not read skill file: " + node.sourcePath);
          }
        }
      }
          ...
          for (const skillId of node.assignedSkills) {
            ...
            if (skillNode?.kind === "skill" && skillNode.sourcePath && !skillFiles.has(skillNode.sourcePath)) {
              try {
                ...
                logger.warn("TreeStore", "Could not read assigned skill file: " + skillNode.sourcePath);
              }
            }
          }
        }
    ...
    if (projectPath && skillFiles.size > 0) {
      for (const [relativePath, content] of skillFiles) {
        try {
          ...
          logger.warn("TreeStore", "Failed to write skill file " + relativePath + ": " + String(err));
        }
      }
    }
  ...
  initRemoteSync(): () => void {
    ...
    const unsubReconnect = remoteSync.onReconnect(() => {
      ...
      logger.info("RemoteSync", "Sent full resync after reconnect");
    });
