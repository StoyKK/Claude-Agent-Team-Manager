...
export const useUiStore = create<UiStore>()((set, get) => ({
  ...
  async loadRemoteConfig(projectPath: string) {
    try {
      ...
      logger.warn("UiStore", "Failed to load remote config: " + String(err));
    }
  },
  ...
  async saveRemoteConfig(projectPath: string) {
    try {
      ...
      logger.warn("UiStore", "Failed to save remote config: " + String(err));
    }
  },
  ...
  async connectRelay(relayUrl: string) {
    try {
      ...
      logger.warn("UiStore", "Failed to connect to relay: " + String(err));
      remoteSync.dispose();
      throw err;
    }
  ...
  async connectRelayPersistent(relayUrl: string) {
    try {
      ...
      logger.warn("UiStore", "Failed to connect to relay (persistent): " + String(err));
      remoteSync.dispose();
      throw err;
    }
