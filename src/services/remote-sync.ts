...
export class CryptoSession {
  ...
  decrypt(envelope: EncryptedEnvelope): RemoteMessage | null {
    ...
    if (!plaintext) {
      logger.warn("CryptoSession", "Decryption failed - invalid ciphertext or nonce");
      return null;
    }
...
class RemoteSyncService {
  ...
  broadcastEvent(type: ServerEventType, payload: unknown): void {
    ...
    if (this._mode === "cloud") {
      ...
      invoke("broadcast_to_remote", {
        ...
        logger.warn("RemoteSync", "Failed to broadcast: " + String(err));
      });
    }
  }
  ...
  private broadcastViaRelay(type: ServerEventType, payload: unknown): void {
    ...
    try {
      ...
      invoke("send_to_relay", { data: JSON.stringify(envelope) }).catch((err) => {
        logger.warn("RemoteSync", "Failed to send via relay: " + String(err));
      });
    } catch (err) {
      logger.warn("RemoteSync", "Encryption failed: " + String(err));
    }
  }
  ...
  async init(): Promise<void> {
    ...
    const u1 = await listen<{ port: number; url: string; pin: string }>(
      ...
      (event) => {
        logger.info("RemoteSync", "Server started on port " + event.payload?.port);
        ...
        for (const handler of this.reconnectHandlers) {
          try {
            ...
            logger.warn("RemoteSync", "Reconnect handler error: " + String(err));
          }
        }
      },
    );
    ...
    const u2 = await listen("remote-server-stopped", () => {
      logger.info("RemoteSync", "Server stopped");
    ...
    const u3 = await listen("remote:request-sync", () => {
      ...
      for (const handler of this.pushHandlers) {
        try {
          ...
          logger.warn("RemoteSync", "Push handler error: " + String(err));
        }
      }
    });
  ...
  async initRelay(relayUrl: string): Promise<RelayStatus> {
    ...
    const u1 = await listen<{ room_code: string }>("relay:room-created", (event) => {
      logger.info("RemoteSync", "Relay room created: " + event.payload.room_code);
    ...
    const u2 = await listen<{ mobile_public_key: string }>("relay:peer-joined", (event) => {
      logger.info("RemoteSync", "Peer joined relay room - awaiting PIN auth");
      if (this._crypto) {
    ...
    const u3 = await listen("relay:peer-disconnected", () => {
      logger.info("RemoteSync", "Peer disconnected from relay");
    ...
    const u4 = await listen("relay:disconnected", () => {
      logger.info("RemoteSync", "Disconnected from relay");
      this._connected = false;
    ...
    const u5 = await listen<{ data: string }>("relay:message", (event) => {
      ...
      try {
        ...
        logger.warn("RemoteSync", "Failed to process relay message: " + String(err));
      }
    });
    ...
    try {
      ...
      logger.warn("RemoteSync", "Failed to connect to relay: " + String(err));
      this._relayStatus.connected = false;
  ...
  async initRelayPersistent(relayUrl: string): Promise<RelayStatus> {
    ...
    const u1 = await listen("relay:registered", () => {
      logger.info("RemoteSync", "Desktop registered with relay for persistent pairing");
      this._relayStatus.connected = true;
      this._connected = true;
      this.notifyConnectionChange();
    });
    ...
    const u2 = await listen<{ mobile_public_key: string; pairing_id?: string }>("relay:peer-joined", (event) => {
      logger.info("RemoteSync", "Peer joined via pairing");
      if (this._crypto) {
        this._crypto.deriveSharedKey(event.payload.mobile_public_key);
      }
      this._relayStatus.clientConnected = true;
    ...
    try {
      ...
      logger.warn("RemoteSync", "Failed to register with relay: " + String(err));
      this._relayStatus.connected = false;
      throw err;
    }
  ...
  async issuePairingToken(deviceName: string = "Mobile Device"): Promise<PairingToken | null> {
    ...
    try {
      ...
      logger.warn("RemoteSync", "Failed to issue pairing token: " + String(err));
      return null;
    }
  }
  ...
  async listPairedDevices(): Promise<PairedDevice[]> {
    ...
    try {
      ...
      logger.warn("RemoteSync", "Failed to list paired devices: " + String(err));
      return [];
    }
  }
  ...
  async revokePairing(pairingId: string): Promise<boolean> {
    try {
      ...
      logger.warn("RemoteSync", "Failed to revoke pairing: " + String(err));
      return false;
    }
  }
  ...
  async syncFullState(
    ...
    await invoke("sync_state_to_remote", {
      ...
      logger.warn("RemoteSync", "syncFullState failed: " + String(err));
    });
  }
  ...
  private async handleAuthMessage(msg: RemoteMessage): Promise<void> {
    ...
    try {
      ...
      if (typeof submittedPin === "string" && submittedPin === desktopPin) {
        this._authenticated = true;
        logger.info("RemoteSync", "PIN auth succeeded");
        ...
        if (this._relayStatus.persistentMode || this._relayStatus.desktopDeviceId) {
          try {
            ...
            logger.info("RemoteSync", "Pairing token issued to mobile");
          } catch (err) {
            logger.warn("RemoteSync", "Failed to auto-issue pairing token: " + String(err));
          }
        }
        ...
        logger.warn("RemoteSync", "PIN auth failed - incorrect PIN");
      ...
      logger.warn("RemoteSync", "Auth handling error: " + String(err));
    }
  }
  ...
  private notifyConnectionChange(): void {
    for (const handler of this.connectionHandlers) {
      try {
        ...
        logger.warn("RemoteSync", "Connection handler error: " + String(err));
      }
    }
  }
}
